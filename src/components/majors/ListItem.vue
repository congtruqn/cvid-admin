<template>
  <div>
    <section class="content-header">
      <h1>Quản lý chuyên nghành</h1>
      <ol class="breadcrumb">
        <li>
          <a><i class="fa fa-dashboard"></i>Trang chủ</a>
        </li>
        <li><a>Danh sách chuyên nghành</a></li>
      </ol>
    </section>
        ...
        >
          ...
          <template v-slot:cell(actions)="{ item }">
            ...
            <b-icon
              ...
              variant="danger"
              style="margin: auto"
              @click="deleteItem(item._id, item)"
            ></b-icon>
          </template>
        </b-table>
      </div>
...
export default {
  data() {
    return {
      ...
      level: 'Đại học',
      levelOptions: ['Phổ thông','Trung cấp','Cao đẳng','Đại học','Chuyên gia'],
  ...
  methods: {
    ...
    callbackMajor() {
      this.items = this.majors.filter(item => item.level === this.level)[0].skills
    }
